...
A Django project template with a strict domain-driven architecture, [django-cotton](https://django-cotton.com) + [htmx](https://htmx.org) (+ [Alpine.js](https://alpinejs.dev)) for the frontend, and [l4py](https://pypi.org/project/l4py/) for structured logging.
...
config/                  Django project configuration
    settings/
        ...
        production.py     DEBUG=False, Postgres, whitenoise, gunicorn
...
Runs `web` (runserver, dev settings) against SQLite with no external
services required.
...
Runs `web` (gunicorn) against Postgres. The production Docker image is built
in stages (Tailwind CSS via Node, Python dependencies, final slim runtime) so
the shipped image has no Node toolchain or dev dependencies.
